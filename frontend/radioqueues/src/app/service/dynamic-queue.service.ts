...
export class DynamicQueueService {
	...
	private async createMusicFileList(queueTypeName: string, percentage: number) {
		...
		let list = await this.createMusicFileListFromFolder(folder);
	...
	private async createMusicFileListFromFolder(folder?: string) {
		if (!folder) {
			return [];
		}
	...
	private getFilesNotRecentlyPlayed(musicFiles: Item[], percentage: number): Array<Item> {
